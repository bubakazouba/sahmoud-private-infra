...
def payload():
    ...
    items = _rows("SELECT date, ts, msg_id, food, qty, cal, protein, fat, carbs, basis,"
...
def _fresh_messages():
    """Load the msg_id->text cache the drilldown builder maintains, refreshing it (via that
    same builder, which also writes messages.json) when missing or >15 min stale so the new
    'message' column in the Items table is current. ONE source for the message text (DRY)."""
    cache = CHAT / "state" / "dashboards" / "calorie-log" / "messages.json"
    builder = CHAT / "scripts" / "build_calorie_drilldown.py"
    try:
    ...
    if stale:
        try:
            subprocess.run([sys.executable, str(builder), "--days", "30"],
                           cwd=str(CHAT), timeout=120, capture_output=True)
        except Exception:
...
def _charts_page():
    return (PAGE.replace("__DATA__", json.dumps(payload()))
                .replace("__MSGS__", json.dumps(_fresh_messages())))


@app.get("/")
def index():
    # Sahmoud 2026-08-18: RESTORE the original charts + bottom Items table as the main view,
    # with the TG messages FOLDED INTO that existing table (a 'message' column) -- NOT a
    # separate drilldown page. ("I just wanted the tg msgs folded in the table that already
    # existed.") The standalone drilldown stays reachable at /drilldown.
    return _charts_page()
...
# Per-day message-to-calorie drill-down (Sahmoud 2026-08-18: "select a day then show
# the map of tg msg to calories and how it was calculated"). The page is generated by
# chat-assistant/scripts/build_calorie_drilldown.py, which reads the SAME DB this app
# reads plus a cached pull of the raw food-channel message text. Served live here; the
# generator is re-run (last 30 days) on each load if the cached file is stale, so past
# days are always current and only messages newer than the last cache pull show no text.
DRILL_HTML = CHAT / "state" / "dashboards" / "calorie-log" / "index.html"
DRILL_BUILDER = CHAT / "scripts" / "build_calorie_drilldown.py"


@app.get("/drilldown")
def drilldown():
    # Refresh if missing or older than 15 min (cheap; the Telethon pull is cached).
    try:
        stale = (not DRILL_HTML.exists()) or (time.time() - DRILL_HTML.stat().st_mtime > 900)
    except OSError:
        stale = True
    if stale:
        try:
            subprocess.run([sys.executable, str(DRILL_BUILDER), "--days", "30"],
                           cwd=str(CHAT), timeout=120, capture_output=True)
        except Exception:
            pass  # fall through and serve whatever we have
    if DRILL_HTML.exists():
        return DRILL_HTML.read_text(encoding="utf-8")
    return ("<h1>Calorie drill-down</h1><p>Not generated yet. Run "
            "scripts/build_calorie_drilldown.py.</p>"), 503
...
td.msgc{white-space:normal;max-width:320px;vertical-align:top;border-bottom:2px solid var(--line)}
.msgtxt{color:var(--ink);font-size:12.5px}
.msgmeta{color:var(--ink2);font-size:11px;margin-top:2px}
.est{color:var(--ink2)}
...
<div class=card><h2>Items - each traced to Preksha's message</h2><div class=scroll id=tbl></div></div>

<script>
const DATA = __DATA__;
const MSGS = __MSGS__;   // msg_id(str) -> {text,time,who,photo}: the verbatim TG message
...
function table(date){
  ...
  // The TG message is folded in as the first column: shown once per message (the items it
  // produced sit under it), so you can trace every calorie back to what Preksha actually
  // typed -- in the same table, not a separate page (Sahmoud 2026-08-18).
  let h="<table><thead><tr><th>message</th><th>time</th><th>food</th><th>qty</th>"+
    "<th class=n>cal</th><th class=n>P</th><th class=n>F</th><th class=n>C</th>"+
    "<th>basis</th><th>who</th><th>day</th></tr></thead><tbody>";
  let prevMsg="__start__";
  for(const i of its){
    const warn=(i.basis||"").indexOf("unparsed")>=0;
    const notHers=(i.person||"preksha")==="sahmoud";
    const mid=(i.msg_id==null?null:String(i.msg_id));
    let msgcell="";
    if(mid!==prevMsg){                       // first item of a new message -> show the message
      const m = mid!=null ? MSGS[mid] : null;
      if(m){
        const t = esc(m.text||"") || '<span class=est>(photo, no caption)</span>';
        msgcell = `<div class=msgtxt>${t}</div>`+
                  `<div class=msgmeta>${esc(m.time||"")} ${esc(m.who||"")}`+
                  `${m.photo?" &#128247;":""}</div>`;
      } else if(mid!=null){
        msgcell = `<span class=est>(msg ${esc(mid)} — text not cached yet)</span>`;
      } else {
        msgcell = '<span class=est>(no source message)</span>';
      }
      prevMsg=mid;
    }
    h+=`<tr${notHers?' class=est':''}><td class=msgc>${msgcell}</td>`+
       `<td>${esc(i.ts||"")}</td><td>${esc(i.food||"")}</td>`+
       `<td>${esc(i.qty||"")}</td>`+
       ...
       `${esc(i.basis||"")}</span></td>`+
       `<td>${esc(i.person||"")}</td><td>${esc(i.day_label||"")}</td></tr>`;
  }
